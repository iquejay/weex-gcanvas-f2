...
const webConfig = {
  ...
  resolve: {
    ...
    alias: {
      '@': helper.resolve('src'),
      'images': helper.resolve('src/images'),
      'platform': helper.resolve('src/platform/web')
    }
  },
  resolveLoader: {
    modules: [helper.resolve('configs/loaders'), 'node_modules']
  },
  /*
   * Options affecting the resolving of modules.
   *
    ...
    rules: useEslint.concat([
      {
        ...
        use: [{
          ...
          loader: 'weex-conditional-compile-loader',
          options: {
            isDebug: process.env.NODE_ENV === 'development', // optional, this is default
            isNative: false,
            isWeb: true // any name, used for /* IFTRUE_myFlag ...js code... FITRUE_myFlag */
          }
        }],
...
const weexConfig = {
  ...
  resolve: {
    ...
    alias: {
      '@': helper.resolve('src'),
      'images': helper.resolve('src/images'),
      'platform': helper.resolve('src/platform/native')
    }
  },
  resolveLoader: {
    modules: [helper.resolve('configs/loaders'), 'node_modules']
      ...
      },
      {
        ...
        use: [{
          ...
          options: {
            isDebug: process.env.NODE_ENV === 'development', // optional, this is default
            isNative: true,
            isWeb: false // any name, used for /* IFTRUE_myFlag ...js code... FITRUE_myFlag */
          }
        }],
        exclude: config.excludeModuleReg
      }
